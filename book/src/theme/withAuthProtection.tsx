...
const WithAuthProtection: React.FC<WithAuthProtectionProps> = ({
  ...
  // Check if this is Urdu locale and authentication is required
  const isUrduLocale = locale === 'ur';
  const needsAuth = isUrduLocale && requireAuth;
  ...
  // If authenticated or not requiring auth, render the original content
  return <>{children}</>;
};
